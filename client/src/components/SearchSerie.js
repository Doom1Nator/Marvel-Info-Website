...
const SearchSerie = (props) => {
	const searchTerm = useSelector((state) => state.searchTerm);
	const { loading, error, data } = useQuery(queries.SEARCH_SERIES, {
		variables: {
	...
	} else if (data && data.searchSeries) {
		dispatch(actions.isloaded());
		dispatch(actions.changeSearchData(data.searchSeries));
	}
