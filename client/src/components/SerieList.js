...
const SerieList = (props) => {
	...
	const { loading, error, data } = useQuery(queries.GET_SERIES, {
		variables: {
			pageNum: pageNum
		}
	});
	...
	if (data && data.getSeries) {
		dispatch(actions.changeShowingData(data.getSeries));
	}
					...
					<CardActionArea>
						<Link to={`/series/${series.id}`}>
							<CardMedia
								...
								component='img'
								image={series.thumbnail ? `${series.thumbnail.path}/portrait_incredible.${series.thumbnail.extension}` : noImage}
								title='serie image'
							/>
							...
							<CardContent>
								...
								<Typography variant='body2' color='textSecondary' component='p'>
									{series.description ? series.description.replace(regex, '').substring(0, 150) + '...' : 'No Description   '}
								</Typography>
							</CardContent>
						</Link>
					</CardActionArea>
	...
	if (Number(pageNum) === 0) {
		pagination =
			<div className='pagiDiv'>
				<Link className='paginations' onClick={() => setPageNum(pageNum + 1)} to={`/series/page/${pageNum + 1}`} >Next page</Link>
			</div>
	} else if (Number(pageNum) === 74) {
		pagination =
			<div className='pagiDiv'>
				<Link className='paginations' onClick={() => setPageNum(pageNum - 1)} to={`/series/page/${pageNum - 1}`} >Previous page</Link>
			</div>
	} else if (Number(pageNum) > 74) {
		pagination =
			<div>
	...
	if (data) {
		if (searchTerm && searchTerm.trim() !== '') {
			if (searchLoading) { //Loading
				return (
					<div>
						<h2><SearchSerie searchValue={searchValue} /></h2>
				...
				if (searchData.length === 0) { //loaded but no results
					return (
						<div>
							...
							<button className="stop-search-button" onClick={() => stopSearching()}>Stop searching</button>
							<br />
							<h2>No such serie, please try another search term!</h2>
						</div >
					);
				} else {
